...
**Phase**: Phase 1 (Step 0~3) — Step 0 실험 통과, 사용자 리뷰 대기
**완료**: Phase 0 전체 ✅ + Step 0 (HF eager forward 결정론) invariant 0.1/0.2 통과 ✅
**Next**: Step 0 사용자 리뷰 → 승인 시 `main`에 `--no-ff` merge + tag `step_00_done` + 브랜치 삭제 → Step 1 진입
**Next task file**: `tasks/step_01_*.md` (Step 0 리뷰 승인 후)
**Branch**: `step/step_00_determinism_check` (리뷰 승인 후 merge·삭제)
...
|---|---|---|---|---|
| 0 | HF eager forward 결정론 | 🔵 | step_00_determinism_report.md | invariant 0.1/0.2 ✅, 리뷰 대기 |
...
Step 0 실험 통과 (invariant 0.1/0.2 ✅), `step/step_00_determinism_check` 브랜치에 commit 완료. 남은 것:

1. **사용자 리뷰** — `docs/reports/step_00_determinism_report.md` 검토
2. 승인 시: `git checkout main && git merge --no-ff step/step_00_determinism_check` → `git tag step_00_done` → `git push origin main step_00_done` → step 브랜치 삭제(로컬+원격)
3. Step 1 진입 (`tasks/step_01_*.md` — Our layerwise forward = HF 표준 forward, no cache)

로컬 A100 교차검증(invariant 0.3)은 미수행 — 사용자 결정 시 별도 round.

## 다음 세션 첫 행동

- Step 0 사용자 리뷰 대기. 승인 시 `main`에 `--no-ff` merge + tag `step_00_done` + 브랜치 삭제 → Step 1 진입
