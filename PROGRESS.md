...
**Phase**: Phase 1 (Step 0~3) — Step 0 진행 중
**완료**: Phase 0 전체 ✅ (0-A MacBook 셋업, 0-D LMCache pinning, 사용자 리뷰 승인 2026-05-14)
**Next**: Step 0 (HF eager forward 결정론 검증). Phase 0-B (vast.ai 셋업)는 Step 0 작업 중 `scripts/vast_helper.py`로 자동 수행.
**Next task file**: `tasks/step_00_determinism_check.md`
**Branch**: `step/step_00_determinism_check` (3-4에서 생성)
...
Phase 0 완료 (사용자 리뷰 승인 2026-05-14). Step 0 진행 중 — `step/step_00_determinism_check` 브랜치:

1. `scripts/vast_helper.py` 4개 함수 구현 (allocate_instance / setup_instance / destroy_instance / ssh_alias_register)
2. 첫 vast.ai 인스턴스 할당 + Phase 0-B 자동 셋업:
   - 인스턴스 할당 (A100-SXM4 80GB) → `~/.ssh/config` `Host vast` 자동 등록
   - `setup/install_vastai.sh` → `.env` 전송 (HF_TOKEN, GITHUB_PAT) → 모델 다운로드 / Loong manifest / `check_env.py` (vastai) / `sanity_forward`
3. `tasks/step_00/run_determinism_check.py` 생성 (task 파일에 전체 코드 명세됨)
4. vast.ai에서 실행 → 결과 git push → 인스턴스 destroy
5. `docs/reports/step_00_determinism_report.md` 작성 → 사용자 리뷰

로컬 A100 검증은 이번 step 미수행 — vast.ai 단독 진행. 환경 간 비교는 사용자 결정 시 별도 round.

## 다음 세션 첫 행동

- Step 0 진행 중 (`step/step_00_determinism_check`). "다음 행동" 1번부터 — `scripts/vast_helper.py` 구현
