...
2. **vast.ai 환경 검증**: `ssh vast 'cd compblend3 && python scripts/check_env.py'` 모든 항목 OK
3. **SSH alias 동작**: `ssh vast 'echo hello'` 가 비밀번호 없이 즉시 응답
4. **모델 로드**: vast.ai에서 Mistral-7B-Instruct-v0.2 로드되어 sample forward 1회 성공
5. **Loong manifest**: vast.ai의 `data/manifests/loong_level1_eng_academic_16k.json` n_instances ≥ 10
6. **LMCache pinning 확정**: `notes/lmcache_pinning.md` 에 commit SHA + vLLM 버전 + patch diff 기록
7. **사용자 리뷰 승인**
...
### Phase 0-B: vast.ai 셋업 (Claude가 ssh로 트리거)
...
- `patches/lmcache-vllm-cacheblend.patch` — vLLM patch diff
...
- [ ] vast.ai `env_check.json` 전부 OK
- [ ] vast.ai `sanity_forward.json` top1 token 합리적
- [ ] Loong manifest n_instances ≥ 10
- [ ] `notes/lmcache_pinning.md` + `patches/lmcache-vllm-cacheblend.patch` commit됨
- [ ] 사용자 리뷰 승인

사용자에게 다음 요청:
> "Phase 0 완료. 3머신 셋업 됨. MacBook + vast.ai env check 통과. Mistral 로드 정상. Loong manifest N=___ 생성. LMCache pinning 확정 (commit SHA ___). Step 0 진행해도 될까요?"
...
- vast.ai SSH alias 설정은 사용자 책임. Claude가 vast.ai 콘솔에서 직접 정보 가져올 수 없음.
