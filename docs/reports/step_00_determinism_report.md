...
- KV cache 미사용 (plain forward). DynamicCache는 Step 2에서 도입.
...
- ⚠️ **환경 간 결정론(0.3) 미검증**: vast.ai ↔ local_a100 SHA-256 일치 여부는 확인하지 않았다. cuDNN/driver 차이로 갈릴 수 있으며, 그 경우 atol 1e-5 fallback이 필요. Step 0 task 파일은 이 검증을 "강력 권장"으로 둠 — 보류 항목.
