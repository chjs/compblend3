...
읽은 후, ssh vast 'nvidia-smi' 로 vast.ai 인스턴스가 살아있는지 확인하고,
다음 행동을 제안해주세요. 진행 전 제 승인을 기다려주세요.
```
...
vast.ai에서 실험 진행 중에 끊겼다면 그것도 PROGRESS.md에 적혀 있어야 한다.
...
3. ssh vast 'cd compblend3 && git status' — vast.ai에서 미완 변경사항
4. ssh vast 'tmux list-sessions' — vast.ai에서 백그라운드로 돌고 있는 실험이 있는지
...
- "vast.ai 인스턴스가 살아있나요? ssh vast 'echo' 시도해도 되나요?"
- "이전 step의 결과를 로컬 A100에서도 재현하셨나요? compare_results.py 결과는?"
- "PROGRESS.md에 적힌 next step으로 바로 진행할까요?"
- "참조 저장소(LMCache, KVzip, Loong)는 어디에 clone되어 있나요? vast.ai의 ~/Loong 인가요?"
- "MacBook의 `~/.ssh/config`에 `Host vast` alias 설정 되어 있죠?"
