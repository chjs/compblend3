...
- **2026-05-16 v13**: Step 2 invariant 2.3 — C-3~C-7 진단 누적 결과 반영, 명세를 2.3A·2.3B로 분리 (옵션 B)
  - **원인 확정 (C-7 F2: M=7 동일 GEMM shape에서 q_proj first 6 row max_abs=0 bitwise)**: cuBLAS shape-dependent kernel dispatch가 `q_proj` (out_dim=4096) GEMM의 reduction 순서를 input shape (M)에 따라 다르게 만든다. 같은 input row에 같은 weight라도 M=6 GEMM과 M=7 GEMM의 output bit-level이 다르다. "buggy code" ❌, "잘못된 shape 사용" ✅.
  - **2.3A (재정의)** = padded forward(length 7, mask=[1,1,1,1,1,1,0], use_cache=True) vs single forward(length 7, mask=[1]*7, use_cache=True)의 DynamicCache K/V[:6] bitwise. Gate = `torch.equal`. atol fallback ❌. forward hook ❌, `output_attentions=True` ❌. `past_key_values[i].key_cache/value_cache` 직접 접근. **둘 다 단일 forward call + use_cache=True**로 DynamicCache가 empty에서 시작 → 7개 K/V append → attention shape `(Q=7, K=7)` 양쪽 동일 → same-shape 비교라 bitwise 가능.
  - **2.3B (신규)** = 운영 split forward (prefill 6 + M=1 decode)의 drift 측정. Gate ❌, 측정만. `max_abs_diff`, `mean_abs_diff`, `argmax_match`, `topk_overlap(k=5)`, `drift_budget_exceeded(max>1e-4)`. cuBLAS shape-dependent dispatch의 정량화. 운영 시 예상 noise floor. 1e-4 threshold는 regression monitoring 용도, 통과 조건 ❌.
  - `all_invariants_passed = 2.1 PASS AND 2.2 PASS AND 2.3A PASS` (2.3B 제외).
  - **Masked dummy slot K/V 처리**: padded path의 position 6 masked dummy slot K/V는 DynamicCache에 들어가나 비교 대상 ❌. 향후 CacheBlend chunk artifact 저장 시 반드시 버린다. (참고: 여기서 "masked dummy slot"은 `input_ids[6] = next_token_id` + `mask=0`으로 구성된 격리된 위치이며 tokenizer-level PAD token이 아니다 — task §2.3A 정당화 단락 참조).
  - **이전 round 분석 정정**: 2026-05-16 작업 0(초기)에서 "옵션 B는 attention K_total=13 vs 7 → bitwise 불가능"이라 결론냈으나, 그건 다른 구조(`prefill(6) cache + padded decode(7)` **2단계**)에 대한 분석이었다. 옵션 B는 **단일 padded forward(7) + use_cache=True** 구조로 cache가 empty 시작이라 attention shape `(7, 7)` 양쪽 동일. mechanism justification은 `tasks/step_02_dynamic_cache.md` §2.3A에 명시.
  - **1e-6 threshold 변경 ❌**: 사용자 전제(연산 순서·정밀도 통제 시 A=B)와 충돌하는 atol 완화는 거부. 명세를 두 부분으로 분리하여 bitwise gate 보존 + drift 측정 분리.
  - **자동 선례화 ❌**: 이 분리는 cross-shape mechanism이 있는 invariant에만 적용. 같은 패턴을 다른 step에 자동 적용하는 선례 ❌.
  - **CacheBlend chunk padding 사전 가정 (Step 4 작업 0에서 검증 후 확정)**: (a) right-padding 사용 (left-padding ❌ — position/RoPE 어긋남 위험), (b) padded_len = chunk_size 또는 bucket size (full prompt length ❌ — CacheBlend 성능 이점 상실), (c) 저장 시 real token K/V만 (PAD K/V 버림, attention_mask=0), (d) Step 4 작업 0에서 chunk 간 GEMM path 동등성 검증으로 정책 확정. 근거: C-4/C-7이 확정한 GEMM shape-dependent dispatch의 직접적 귀결.
